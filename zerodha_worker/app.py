...
def get_api_key_token():
    return jsonify({"api_key": api_key, "access_token": access_token})
...
def place_market_buy_order():
    ...
    try:
        ...
        try:
            ...
            live_price = kite.ltp(f"{exchange}:{ticker}")[f"{exchange}:{ticker}"][
                "last_price"
            ]
            ...
            return jsonify({"message": f"order is placed"}), 200
...
def place_market_sell_order():
    ...
    try:
        ...
        try:
            ...
            return jsonify({"message": f"order id is placed"}), 200
        except Exception as ex:
            print(ex)
            return jsonify({"error": str(ex)}), 500
...
def get_historical_data():
    ...
    historical_data = kite.historical_data(
        data["token"], data["fdate"], data["tdate"], data["interval"], False, False
    )
...
def get_ema(symbol):
    ...
    df = requests.post(
        "http://zerodha_worker_index/get/historical_data",
        json={
            "fdate": str(fday),
            "tdate": str(tday),
            "token": token,
...
def get_rsi(symbol, n):
    ...
    try:
        ...
        return jsonify({"last_rsi": last_rsi, "last_slope": last_deg})
    except:
        return jsonify({"last_rsi": 0, "last_slope": 0})
...
def get_obv(symbol):
    ...
    df = requests.post(
        f"http://zerodha_worker_index/get/historical_data",
        json={
            "fdate": str(fday),
            "tdate": str(tday),
            "token": token,
...
def ema_5813(symbol):
    ...
    print(
        "ema for 581320 are" + "" + "" + symbol + str(ema_5),
        str(ema_8),
        str(ema_13),
        str(ema_20),
    )
    print(
        "slopes of 581320 are" + "" + "" + symbol + str(slope_5),
        str(slope_8),
        str(slope_13),
        str(slope_20),
    )
...
def slope(symbol, n):
    ...
    return jsonify({"slope": slope, "trend": trend})
