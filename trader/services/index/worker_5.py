        ...
        while True:
            for ticker in self.tickers:
                ...
                for strike in ce_documents:
                    ...
                    for strike_ in pe_documents:
                        ...
                        if ltp_pe >= min_ltp_ce and ltp_pe <= max_ltp_ce:
                            pairs.add(
                                (
                                    ce_documents[strike]["monthly_Options_CE"],
                                    pe_documents[strike_]["monthly_Options_PE"],
                                )
                            )
                ...
                for strike in pe_documents:
                    ...
                    for strike_ in ce_documents:
                        ...
                        if ltp_ce >= min_ltp_pe and ltp_ce <= max_ltp_pe:
                            pairs.add(
                                (
                                    ce_documents[strike_]["monthly_Options_CE"],
                                    pe_documents[strike]["monthly_Options_PE"],
                                )
                            )
                ...
                for pair in pairs:
                    ...
                    trade_a = self.generateMarketOrderBuyIndexOption(
                        ticker_a, self.quantity, "ENTRY"
                    )
                    trade_b = self.generateMarketOrderBuyIndexOption(
                        ticker_b, self.quantity, "ENTRY"
                    )
    ...
    def exitStrategy(self):
        while True:
            for ticker_pair in self.ticker_pairs:
                ...
                if pnl_a + pnl_b >= 4:
                    trade_a = self.generateMarketOrderSellIndexOption(
                        ticker_a, self.quantity, "EXIT"
                    )
                    trade_b = self.generateMarketOrderSellIndexOption(
                        ticker_b, self.quantity, "EXIT"
                    )
