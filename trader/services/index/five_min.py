...
class fiveminindex(TradeApp):
    ...
    def entryStrategy(self):
        ...
        while True:
            ...
            if nifty_live['last_price']>self.nifty_5min['high'] and nifty_slope>0 and datetime.datetime.now().time()>datetime.time(9,25): #and nifty_live['last_price']>self.nifty_5min['high']:
            ...
            if nifty_live['last_price']<self.nifty_5min['low'] and nifty_slope<0 and datetime.datetime.now().time()>datetime.time(9,25):
                ticker=self.data['index']['NSE:NIFTY 50']['pe_ticker']
                trade=self.generateMarketOrderBuyIndexOption(ticker,50,'ENTRY')
                #the below line sends trade to front end
                self.sendTrade(trade)

            if banknifty_live['last_price']>self.banknifty_5min['high'] and banknifty_slope>0 and datetime.datetime.now().time()>datetime.time(9,25):
                ticker=self.data['index']['NSE:NIFTY BANK']['ce_ticker']
                trade=self.generateMarketOrderBuyIndexOption(ticker,25,'ENTRY')
                #the below line sends trade to front end
                self.sendTrade(trade)

            if banknifty_live['last_price']<self.banknifty_5min['low'] and banknifty_slope<0 and datetime.datetime.now().time()>datetime.time(9,25):
