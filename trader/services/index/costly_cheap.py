...
class CostlyCheap(TradeApp):
    ...
    def getCostlyCheap(self):
        ...
        ce_ticker = (
            "NSE:NIFTY" + self.year + self.month + self.week + str(ce_price) + "CE"
        )
        pe_ticker = (
            "NSE:NIFTY" + self.year + self.month + self.week + str(pe_price) + "PE"
        )

        if TRADE_ENV == "prod":
            ce_live = self.getLiveData(ce_ticker)
            pe_live = self.getLiveData(pe_ticker)
        else:
            ce_live = {"last_price": random.randint(1, 100)}
            pe_live = {"last_price": random.randint(1, 100)}
        ...
        while True:
            ...
            for order in orders:
                ...
                if TRADE_ENV == "prod":
                    ...
                    ticker_live = {"last_price": random.randint(1, 100)}
