        ...
        while True:
            ...
            if now >= datetime.time(9, 30):
                ...
                for ticker in self.tickers:
                    try:
                        live_data=self.getLiveData(ticker) # this will give live data of stock
                ...
                if ((live_data["last_price"]>self.ohlc_ticker[ticker]["high"])
                    and (self.tickers[ticker]["ce_ticker"] not in self.entered_tickers)
                    and self.price_diff(live_ce["depth"]["sell"][0]["price"],live_ce["depth"]["buy"][0]["price"])<5):

                    trade = self.generateLimitOrderBuyStockOption(self.tickers[ticker]["ce_ticker"], "ENTRY")
                    self.sendTrade(trade)
                    self.entered_tickers.add(self.tickers[ticker]["ce_ticker"])

                elif ((live_data["last_price"]<self.ohlc_ticker[ticker]["low"])
                    and (self.tickers[ticker]["pe_ticker"] not in self.entered_tickers)
                    and self.price_diff(live_pe["depth"]["sell"][0]["price"],live_pe["depth"]["buy"][0]["price"])<5):

                    trade = self.generateLimitOrderBuyStockOption(self.tickers[ticker]["pe_ticker"], "ENTRY")
                    self.sendTrade(trade)
    ...
    def exitStrategy(self):
        ...
        while True:
            ...
            for order_ in orders:
                derivative=order_["ticker"] # here you will get the derivative of ticker
                ticker=self.derivative_map[derivative] # here you will get the actual ticker
