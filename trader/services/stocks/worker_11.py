        ...
        while True:
            ...
            if now >= datetime.time(9, 30):
                ...
                for ticker in self.tickers:
                    ...
                    if current_volume>1.2*self.tickers[ticker]['avg_volume'] and current_price > self.tickers[ticker]['prev_high'] and buy_quantity>sell_quantity and self.tickers[ticker]['ce_ticker'] not in self.entered_tickers:
                    ...
                    elif current_volume>1.2*self.tickers[ticker]['avg_volume'] and current_price < self.tickers[ticker]['prev_low'] and sell_quantity>buy_quantity and self.tickers[ticker]['pe_ticker'] not in self.entered_tickers:
                        entry_conditions={
                            'current_price': current_price,
    ...
    def exitStrategy(self):
        while True:
            ...
            for order_ in orders:
                ...
                if "CE" in derivative and current_price < master_ticker['ltp'] or pnl >= 10 or datetime.datetime.now().time() >= datetime.time(15, 25):
                ...
                elif "PE" in derivative and current_price > master_ticker['ltp'] or pnl >= 10 or datetime.datetime.now().time() >= datetime.time(15, 25):
                    trade = self.generateLimitOrderSellStockOption(derivative, 'EXIT')
                    self.sendTrade(trade)
                    self.deleteOrder(derivative)
                    # self.entered_tickers.remove(derivative)
