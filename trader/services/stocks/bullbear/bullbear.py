...
class BullBear(TradeBot):
            ...
            for ticks in TickerGenerator("22", "FEB", "", "", "").stocks():
                try:
                    historical_data = self.zerodha.historical_data_today(
                        ticks.ticker.tradingsymbol, HistoricalDataInterval.INTERVAL_5_MINUTE)
                ...
                if ((historical_data[0].open == historical_data[0].low) and ohlcview == "bull") or (view == "bull"):
                    try:
                ...
                if ((historical_data[0].open == historical_data[0].high) and ohlcview == "bear") or (view == "bear"):
                    try:
    ...
    def exit_strategy(self, order: Order):
        ...
        try:
            historical_data = self.zerodha.historical_data_today(self.get_original_ticker(order.trading_symbol), HistoricalDataInterval.INTERVAL_5_MINUTE)
        ...
        if self.option_type(order.trading_symbol) == "CE" and quote.last_price < historical_data[0].low:
            self.exit_trade(trade)
            return
        
        if self.option_type(order.trading_symbol) == "PE" and quote.last_price > historical_data[0].high:
            self.exit_trade(trade)
            return
