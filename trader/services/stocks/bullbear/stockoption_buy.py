...
class StockOptionBuying(TradeBot):
    ...
    def entry_strategy(self):
        while True:
            ...
            for ticks in TickerGenerator("22", "FEB", "", "", "").stocks():
                ...
                try:
                    intraday_data=self.zerodha.historical_data_today(ticks.ticker.tradingsymbol,HistoricalDataInterval.INTERVAL_5_MINUTE,)
                    
                    if len(intraday_data) == 0:
                        raise Exception ("intraday data is not available for" , ticks.ticker.tradingsymbol)
                ...
                view =self.view(first_direction,first_candle_length,first_body_length)
                try:
                ...
                if (
                    ...
                    if (quote.last_price > intraday_data[0].high) and (ticks.ticker.tradingsymbol not in self.fivemin_tickers):                        
                ...
                if (
                    ...
                    if (quote.last_price < intraday_data[0].low) and (ticks.ticker.tradingsymbol not in self.fivemin_tickers):
    ...
    def exit_strategy(self, order: Order):
        ...
        try:
            ...
            if len(intraday_data) == 0:
                raise Exception("empty historical data for original" , order.trading_symbol)
        except Exception as e:
            print(e)
            return
